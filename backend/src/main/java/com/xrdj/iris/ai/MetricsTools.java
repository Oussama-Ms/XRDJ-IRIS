...
public class MetricsTools {
    ...
    public MetricsTools(RuleCounterRecordRepository repository, FileArchiveRepository archiveRepository) {
    ...
    public Function<MetricsRequest, MetricsResponse> getRejectedCREsToday() {
        return request -> {
            ...
            List<String> filesToday = archiveRepository.findAll().stream()
                .filter(archive -> archive.getIngestionDate() != null && archive.getIngestionDate().toLocalDate().equals(today))
            ...
            int totalRejected = records.stream()
                .filter(r -> r.getFileName() != null && filesToday.contains(r.getFileName()))
                .filter(r -> {
                    if (request.flowType() == null || request.flowType().trim().isEmpty() || request.flowType().equalsIgnoreCase("ALL")) {
                        return true; // Don't filter by flow type if ALL is requested
                    }
                    return r.getTypeFlux() != null && r.getTypeFlux().trim().equalsIgnoreCase(request.flowType().trim());
                })
                .mapToInt(r -> {
                    try {
                        // The database stores these as strings, so we parse them safely
    ...
    public String getMetricsSummaryForDate(LocalDate targetDate) {
        ...
        List<String> filesOnDate = archiveRepository.findAll().stream()
            .filter(archive -> archive.getIngestionDate() != null && archive.getIngestionDate().toLocalDate().equals(finalTargetDate))
            .map(FileArchive::getFileName)
            .collect(Collectors.toList());

        List<RuleCounterRecord> records = repository.findAll().stream()
            .filter(r -> r.getFileName() != null && filesOnDate.contains(r.getFileName()))
            .collect(Collectors.toList());
        ...
        return String.format(
            ...
            (creReceived + ecReceived), creReceived, ecReceived,
            (creRejected + ecRejected), creRejected, ecRejected,
            (creTreated + ecTreated), creTreated, ecTreated
        );
    }
    ...
    private long sumField(List<RuleCounterRecord> records, String recordType, String fieldName) {
        return records.stream()
            .filter(r -> r.getRecordType() != null && r.getRecordType().trim().equalsIgnoreCase(recordType))
            .mapToLong(r -> {
                String val = "";
                ...
                try {
                    return val != null && !val.trim().isEmpty() ? Long.parseLong(val.trim()) : 0L;
                } catch (Exception e) {
                    return 0L;
                }
            })
